...
var getViewForSwagger2 = function(opts, type) {
  ...
  _.forEach(swagger.paths, function(api, path) {
    if (opts.exclude.length > 0) {
      const exclude_path = opts.exclude.find(item =>  path.startsWith(item))
      if (exclude_path !== undefined) {
        return
      }
    }
    if (opts.include.length > 0) {
      const include_path = opts.include.find(item =>  path.startsWith(item))
    ...
    _.forEach(api, function(op, m) {
      ...
      _.forEach(params, function(parameter) {
        ...
        parameter.safeName = safeName(parameter.name);
        ...
        if (parameter.in === "body") {
          ...
          // 当前method有一个post参数，就需要为true
          method.hasPostParams = true
        } else if (parameter.in === "path") {
          parameter.isPathParameter = true;
          method.hasPathParams = true
      ...
      _.forEach(responses, function(response, status) {
        method.responses["status" + status] = {
          tsType: ts.convertType(response)
        };
      });
